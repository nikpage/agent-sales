...
export async function proposeActions(inputs: ActionInput[]): Promise<ProposedAction[]> {
  ...
  // 4) Persist (INSERT; ignore duplicates)
  if (deduped.length) {
    ...
    if (data) {
      await Promise.allSettled(
        data.map(async (row) => {
          ...
          const recipient = asNonEmptyString(original.recipient_email);
          if (!requiresEmail || !recipient) return;
            ...
            const rendered = renderEmail(
              {
                ...
                immovability_bonus: original.immovability_bonus,
                context_payload: payload,
                rationale: row.rationale
              },
              actionTemplates,
              recipient,
              `https://yourdomain.com/unsubscribe?action_id=${row.id}`
            );
