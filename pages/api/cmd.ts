...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  /**
   * I'LL DO IT - Update status to handled and mark email as UNREAD again
   */
  async function handleIllDoIt(proposal: { id: string; user_id: string }): Promise<void> {
    ...
    const { data: messages } = await supabase
      .from('messages')
      .select('external_id')
      .eq('conversation_id', (await supabase.from('action_proposals').select('conversation_id').eq('id', proposal.id).single()).data?.conversation_id)
